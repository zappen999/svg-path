...
Path.prototype.convertQuadraticToCubic = function () {
  this.filterMap(function (command, x1, y1, x, y) {
    if (command.type === 'Q') {
      return quadraticToCubicBezier(command, x1, y1, x, y)
    }
  })
}
...
function quadraticToCubicBezier (cmd, absX, absY) {
  var p1 = { x: absX, y: absY }
  ...
  var p2 = { x: cmd.x, y: cmd.y }
  var c = { x: cmd.x1, y: cmd.y1 }

  var c1 = [((2*c.x)+p1.x)/3, ((2*c.y)+p1.y)/3]
  var c2 = [((2*c.x)+p2.x)/3, ((2*c.y)+p2.y)/3]

  var args = c1.concat(c2).concat([p2.x, p2.y])
  return parseCommand('C', args)
}
